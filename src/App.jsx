...
export default function App() {
  ...
  const [posts,                setPosts]                = useState(() => {
    try { return JSON.parse(localStorage.getItem('ssc_posts') || '[]'); } catch { return []; }
  });
  const [composeMovie,         setComposeMovie]         = useState(null);
  ...
  // ── Persist posts ─────────────────────────────────────────────────────────────
  useEffect(() => {
    localStorage.setItem('ssc_posts', JSON.stringify(posts));
  }, [posts]);
  ...
  const openComposer = useCallback((movie = null) => {
    setComposeMovie(movie);
    setActiveTab('blog');
  }, []);
      ...
      <div className="content-area">
        ...
        {activeTab === 'blog' && (
          <BlogTab
            ...
            moviePool={moviePool}
            composeMovie={composeMovie}
            onMovieClick={handleSelectMovie}
            onComposeClear={() => setComposeMovie(null)}
          />
        )}
      </div>
